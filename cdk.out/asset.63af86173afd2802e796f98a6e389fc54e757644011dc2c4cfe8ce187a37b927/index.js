...
var handler = async (event) => {
  ...
  if (dataType === "trips") {
    const res = tripOperation(method, aid, tripId, queryString, body);
    console.log("res from trip op ", res);
    return res;
  } else if (dataType === "people") {
    return peopleOperation(method, aid, personId, queryString, body);
  } else {
...
async function tripOperation(method, aid, tripId, queryString, body = {}) {
  try {
    if (method === "POST") {
      ...
      await trip2.save();
      return { statusCode: 200, body: trip2 };
    }
...
async function peopleOperation(method, aid, personId, queryString, body = {}) {
  try {
    if (method === "POST") {
      ...
      await person2.save();
      return { statusCode: 200, body: person2 };
    }
